...
PROJ_DIR = "/code_path/seq2seq_img_dualkd_mbart/transformers"
...
class DataTrainingArguments:
    ...
    src_lang: Optional[str] = field(default="english", metadata={"help": "Source language id for translation."})
    tgt_lang: Optional[str] = field(default="english", metadata={"help": "Target language id for translation / summarization."})
...
#    print("model_args.model_name_or_path", model_args.model_name_or_path,  model_args.config_name) 
...
#        use_fast=False, cache_dir=model_args.cache_dir, src_lang=data_args.src_lang, tgt_lang=data_args.tgt_lang,
#    )
    lang_map = {"russian": "ru_RU", "english": "en_XX", "urdu": "ur_PK", "indonesian": "id_ID"}
    tokenizer = MBart50TokenizerFast.from_pretrained(model_args.tokenizer_name if model_args.tokenizer_name else model_args.model_name_or_path, src_lang=lang_map[data_args.src_lang], tgt_lang=lang_map[data_args.tgt_lang])
...
#        model.config.decoder_start_token_id = tokenizer.bos_token_id
        self.decoder_start_token_id = tokenizer.lang_code_to_id[data_args.tgt_lang]

        model.config.decoder_start_token_id = self.decoder_start_token_id
        ...
        model = AutoModelForSeq2SeqLM.from_pretrained(
            ...
            config=config,
            cache_dir=model_args.cache_dir,
            use_logit_kd=training_args.use_logit_kd,
            lambda_kd=training_args.lambda_kd,
        ...
        if langid_map is None:
            ...
            with open(data_args.langid_map_path) as f:
                ...
                if isinstance(model, T5ForConditionalGeneration) and data_args.reinitialize_langid_embeddings is not None:
                    ...
                    logger.info(f"set langid input embeddings done!{new_embedding}")
