...
def summarize_xlingual(
    ...
    if os.path.isfile(os.path.join(output_dir, f"{args.data_type}_generations.txt")):
        return
    print("args.xlingual_summarization_model_name_or_path", args.xlingual_summarization_model_name_or_path)
    script_path =  os.path.abspath("/code_path/seq2seq_img_dualkd_mbart/pipeline.py")
    script_args = [
...
def run(args):
    ...
    for pair in tqdm(required_pairs, desc="Running evaluation"):
        ...
        def evaluate(lase_key):
            ...
            root_source_path2 = os.path.join(args.dataset_dir, prefix + source_suffix + "2")
            pipeline_source_path2 = os.path.join(dir_name, source_suffix[1:] + "2")
            ...
            # for image copy
            shutil.copy(
                os.path.join(args.dataset_dir, prefix + '_test.boxes36.h5'),
                os.path.join(dir_name, 'test.boxes36.h5') 
            )
#            print(root_source_path, prefix, source_suffix, pipeline_source_path, dir_name, source_suffix[1:])
#            print(root_target_path, prefix, target_suffix, pipeline_target_path, target_suffix[1:])
            ...
            if args.data_type == "test":
                shutil.copy(
                    pipeline_source_path,
                    os.path.join(dir_name, "val.source")
                )
                shutil.copy(
                    pipeline_source_path2,
                    os.path.join(dir_name, "val.source2")
                )
                shutil.copy(
                    pipeline_source_path,
                    os.path.join(dir_name, "val.target")
                )

            if args.evaluation_type == "xlingual":
                summarize_xlingual(dir_name, dir_name, tgt_lang, src_lang, args)
                ...
                os.remove(os.path.join(dir_name, 'test.boxes36.h5'))
...
#            lase_scores = calculate_lase(pred_lines, ref_lines, tgt_lang)
            scores[lase_key] = 0.0 #lase_scores["LaSE"]
