...
#        print("self.img_file", self.img_file, data_dir, type_path)
        if len(type_path.split("_")[0]) == 2:
            src_lang, tgt_lang = type_path.split("_")[0].split("-")
            print("src_lang tgt_lang", src_lang, tgt_lang)
            self.dataset_kwargs.update({"src_lang": src_lang, "tgt_lang": tgt_lang})
...
class Seq2SeqDataset(AbstractSeq2SeqDataset):
    ...
    def __getitem__(self, index) -> Dict[str, str]:
        ...
        src_lang = self.dataset_kwargs.get("src_lang", None)
...
#        return {"tgt_texts": tgt_line, "src_texts": source_line, "data_id": data_id}
        return {"tgt_texts": tgt_line, "src_texts": source_line, "data_id": data_id, "img_ids": img_id, "img_feature_path": self.image_feature_path, "tgt_texts2": tgt_line2, "src_texts2": source_line2, "src_lang": src_lang}
...
#            self.dataset_kwargs["tgt_lang"] = data_args.tgt_lang
        ...
        #print(" self.is_bert_based -------------",  self.is_bert_based,  self.tokenizer.cls_token)
...
#                    init_token_id = self.tokenizer.convert_tokens_to_ids(mapped_token)
                    lang_map = {"russian": "ru_RU", "english": "en_XX", "urdu": "ur_PK", "indonesian": "id_ID"}
                    init_token_id = self.tokenizer.lang_code_to_id[mapped_token]
                    src_lang_id = self.tokenizer.lang_code_to_id[lang_map[batch[0]["src_lang"]]]
#                    if mapped_token == "id_ID":
#                        init_token_id = 250033
#                    if mapped_token == "ur_PK":
#                        init_token_id = 250050
                else:
                    logger.error(f"Unknown langid: {init_token}")
                #print("-----init_token tgt_lang, ", init_token_id, mapped_token, src_lang_id, batch[0]["src_lang"]) # 3 ▁<extra_id_98>
#        print("init_token_id------", init_token_id, self.tokenizer.convert_ids_to_tokens(init_token_id), self.tokenizer.convert_ids_to_tokens(250053), self.tokenizer.convert_ids_to_tokens(250027)) # 3 <unk>
#        print("test----", self.tokenizer._convert_token_to_id("id_ID"), self.tokenizer._convert_token_to_id("ur_PK"))
#        for idx in range(249990, 250054):
#            print(idx, self.tokenizer.convert_ids_to_tokens(idx), self.tokenizer._convert_token_to_id(self.tokenizer.convert_ids_to_tokens(idx)))
...
#            batch = self._bert_encode_two(batch, init_token_id)
            batch, batch2 = self._encode_two(batch, lang_map[batch[0]["src_lang"]], mapped_token)

            input_ids = batch["input_ids"]
#            print("before:", input_ids)
            index = torch.LongTensor([0])
            input_ids = input_ids.index_fill(1, index, src_lang_id)
#            input_ids[0, :] = src_lang_id
#            print("after:", input_ids)
            attention_mask =  batch["attention_mask"]

            decoder_input_ids = batch["labels"][:, :-1]
#            decoder_input_ids[0, :] = init_token_id
            decoder_input_ids = decoder_input_ids.index_fill(1, index, init_token_id)
...
#            input_ids2[0, :] = init_token_id
            input_ids2 = input_ids2.index_fill(1, index, init_token_id)
            attention_mask2 =  batch2["attention_mask"]

            decoder_input_ids2 = batch2["labels"][:, :-1]
            decoder_attention_mask2 = (decoder_input_ids2 != self.tokenizer.pad_token_id)

            labels2 = batch2["labels"][:, 1:].clone()
            labels2[labels2 == self.tokenizer.pad_token_id] = -100


#            batch.update({"image_features": img, "image_len": img_len, "decoder_start_token_id": init_token_id})
...
#            decoder_input_ids = self._shift_right_t5(batch["labels"], init_token_id)
#            batch["decoder_input_ids"] = decoder_input_ids
#            prit("batch-------------------------------------------", init_token_id)        
            pass
        else:
...
#        prit("batch-------------------------------------------", batch)        
        if self.is_bert_based:
            batch = {
                ...
                "decoder_start_token_id": init_token_id,
...
#        print("batch-------------------------------------------", batch)
    ...
    def _encode_two(self, batch, src_lang, tgt_lang) -> Dict[str, torch.Tensor]:
#        print("batch--------", src_lang, tgt_lang) 
        ...
        self.tokenizer.tgt_lang = src_lang
    ...
    def _bert_encode_two(self, batch, init_token_id):
        inputs = self.tokenizer(
            [x["src_texts"] for x in batch],
            truncation=True,
            max_length=self.data_args.max_source_length,
        ...
        outputs2 = self.tokenizer(
            [x["tgt_texts2"] for x in batch],
            truncation=True,
            max_length=self.data_args.max_target_length,
            padding=self.padding,  # TPU hack
            return_tensors="pt",
        ...
        shifted_input_ids = outputs.input_ids.new_zeros(outputs.input_ids.shape)
        shifted_input_ids[..., 1:] = outputs.input_ids[..., :-1].clone()
        shifted_input_ids[..., 0] = init_token_id
        ...
        output_batch = {
            ...
            "decoder_input_ids2": outputs2.input_ids,
            "decoder_attention_mask2": outputs2.attention_mask,
        }
#        print("init_token_id-0--------------", init_token_id, output_batch)
#        print("shifted_input_ids:", shifted_input_ids)
#        print("ori decoder_input_ids:", outputs.input_ids)
#        print("labels:", labels)
