...
class MBartEncoder(MBartPreTrainedModel):
    ...
    def __init__(self, config: MBartConfig, embed_tokens: Optional[nn.Embedding] = None, fusion_layer=11, use_img_trans=True, use_forget_gate=True, cross_attn_type=4, dim_common=256, n_attn_heads=8, use_logit_kd=True, lambda_kd=0.5, temperature=2, mode="train"):
...
#        if cross_attn_type == 4:
 ...
 #       if use_forget_gate:
    ...
    def forward(
        ...
        #print("self.layers", len(self.layers)) # 12
...
class MBartDecoder(MBartPreTrainedModel):
    ...
    def forward(
        ...
        return_dict = return_dict if return_dict is not None else self.config.use_return_dict
        #print("input_ids in decoder:", input_ids)
        # retrieve input_ids and inputs_embeds
        if input_ids is not None and inputs_embeds is not None:
...
class MBartForConditionalGeneration(MBartPreTrainedModel):
    ...
    def __init__(self, config: MBartConfig, use_logit_kd=True, lambda_kd=0.5, temperature=2, mode="train", use_dual_kd=True):
        super().__init__(config)
    ...
    def forward(
        ...
        if self.mode == "kd_train":
            ...
            lm_logits2 = self.lm_head(outputs2[0]) + self.final_logits_bias
...
#                loss = loss + loss2 # multitask setting
            #KD loss
                ...
                # MSE loss (logit) second best (https://github.com/AberHu/Knowledge-Distillation-Zoo/blob/65470c846c1300d555815da26999cbe2b51fc815/kd_losses/logits.py)
                    masked_lm_loss += F.mse_loss(lm_logits, lm_logits2) * self.lambda_kd # (1.0 default)
                    if self.use_dual_kd:
                        masked_lm_loss += F.mse_loss(lm_logits2, lm_logits) * self.lambda_kd # (1.0 default) inversed
                else:
                    alpha = self.lambda_kd # 0.5
                    T = self.temperature #2
                    KD_loss2 = 0.0
                    masked_lm_loss += alpha * nn.KLDivLoss()(F.log_softmax(lm_logits/T, dim=1), F.softmax(lm_logits2/T, dim=1)) * (alpha * T * T)
                    if self.use_dual_kd:
                        masked_lm_loss += alpha * nn.KLDivLoss()(F.log_softmax(lm_logits2/T, dim=1), F.softmax(lm_logits/T, dim=1)) * (alpha * T * T)
                # soft target
                #kd_loss = F.kl_div(F.log_softmax(out_s/T, dim=1), F.log_softmax(out_t/T, dim=1), reduction='batchmean') * self.T * self.T
#                    masked_lm_loss = masked_lm_loss + (KD_loss + KD_loss2) * alpha
