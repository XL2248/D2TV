...
class T5Stack(T5PreTrainedModel):
    def __init__(self, config, embed_tokens=None, fusion_layer=None, use_img_trans=None, use_forget_gate=None, cross_attn_type=0, dim_common=256, n_attn_heads=1):
...
class T5ForConditionalGeneration(T5PreTrainedModel):
    ...
    def __init__(self, config, fusion_layer=5, use_img_trans=True, use_forget_gate=True, cross_attn_type=0, dim_common=256, n_attn_heads=1, use_logit_kd=True, lambda_kd=0.5, temperature=2, mode="train"):
        super().__init__(config)
        ...
        self.init_weights()
        self.img_linear = nn.Linear(2048, config.d_model) # K
        self.dropout = nn.Dropout(config.dropout_rate)
    ...
    def get_contrastive_loss1(self, image_feat, text_feat, idx=None):
        """
        Args:
    ...
    def get_contrastive_loss(self, image_feat, text_feat, idx=None):
        ...
        image_feat_all = F.normalize(image_feat, dim=-1)
        text_feat_all = F.normalize(text_feat, dim=-1)
        logits = image_feat_all @ text_feat_all.t() / self.temp
    ...
    def forward(
        ...
        if encoder_outputs is None:
            ...
            if self.mode == "train":
                ...
                target_attention_mask = torch.ones(labels.shape, dtype=torch.long, device=labels.device)
                target_attention_mask[labels == 0] = 0
                #print("labels", target_attention_mask, labels)
                target_encoder_outputs2 = self.encoder(
                    input_ids=labels,
        ...
        if self.mode == "train":
            decoder_outputs2 = self.decoder(
                ...
                inputs_embeds=decoder_inputs_embeds,
                past_key_values=past_key_values,
                encoder_hidden_states=hidden_states2,
            ...
            sequence_output2 = decoder_outputs2[0]
        ...
        if self.model_parallel:
            ...
            if self.mode == "train":
                sequence_output2 = sequence_output2.to(self.lm_head.weight.device)
        ...
        if self.config.tie_word_embeddings:
            ...
            if self.mode == "train":
                sequence_output2 = sequence_output2 * (self.model_dim ** -0.5)

        lm_logits = self.lm_head(sequence_output)
        if self.mode == "train":
            lm_logits2 = self.lm_head(sequence_output2)
        ...
        if labels is not None:
            ...
            if self.mode == "train":
#                loss2 = loss_fct(lm_logits2.view(-1, lm_logits2.size(-1)), labels.view(-1))
#                loss = loss + loss2 # multitask setting
            #KD loss
                feat_img = torch.mean(self.img_linear(encoder_outputs["image_features"]), dim=1, keepdim=False)
                feat_img = self.dropout(feat_img)
#                feat_ls = target_encoder_outputs2["last_hidden_state"][:, 0, :]
                mask = (labels != 0)
#            encoder_embedding = (encoder_output * mask.unsqueeze(-1)).sum(dim=1) / mask.float().sum(dim=1).unsqueeze(-1)  # [batch, hidden_size]
                feat_ls = (target_encoder_outputs2["last_hidden_state"] * mask.unsqueeze(-1)).sum(dim=1) / mask.float().sum(dim=1).unsqueeze(-1)

                # Normalize each vector by its norm
                contr_loss1, contr_loss2, contr_loss3 =0, 0, 0
                contr_loss1 = self.get_contrastive_loss(feat_img, feat_ls)
#                contr_loss2 = self.get_contrastive_loss(feat_img, feat_cls)
#                contr_loss3 = self.get_contrastive_loss(feat_mls, feat_cls)
                loss += (contr_loss1 + contr_loss2 + contr_loss3) * self.lambda_kd
                if self.use_logit_kd:
                # MSE loss (logit) second best (https://github.com/AberHu/Knowledge-Distillation-Zoo/blob/65470c846c1300d555815da26999cbe2b51fc815/kd_losses/logits.py)
                    loss += F.mse_loss(lm_logits, lm_logits2) * self.lambda_kd # (1.0 default)
                    loss += F.mse_loss(lm_logits2, lm_logits) * self.lambda_kd # (1.0 default) inversed
                else:
                    alpha = self.lambda_kd # 0.5
                    T = self.temperature #2
                    KD_loss2 = 0.0
                    KD_loss = nn.KLDivLoss()(F.log_softmax(lm_logits/T, dim=1), F.softmax(lm_logits2/T, dim=1)) * (alpha * T * T)
                    KD_loss2 = nn.KLDivLoss()(F.log_softmax(lm_logits2/T, dim=1), F.softmax(lm_logits/T, dim=1)) * (alpha * T * T)
                # soft target
                #kd_loss = F.kl_div(F.log_softmax(out_s/T, dim=1), F.log_softmax(out_t/T, dim=1), reduction='batchmean') * self.T * self.T
                    loss = loss + (KD_loss + KD_loss2) * alpha
